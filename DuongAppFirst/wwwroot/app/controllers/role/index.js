var RoleController = function () {
    ...
    function registerEvents() {
        ...
        $('#btnSave').on('click', function (e) {
            if ($('#frmMaintainance').valid()) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        duong.notify('Update role successfully', 'success');
        ...
        $('body').on('click', '.btn-delete', function (e) {
            e.preventDefault();
            var that = $(this).data('id');
            duong.confirm('Are you sure to delete?', function () {
                $.ajax({
                    type: "POST",
                    ...
                    error: function (status) {
                        duong.notify('An error occurred while deleting', 'error');
                        duong.stopLoading();
                    }
                });
            });
        ...
        $("#btnSavePermission").off('click').on('click', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    duong.notify('Save permission successfully', 'success');
                    $('#modal-grantpermission').modal('hide');
                    duong.stopLoading();
                },
                error: function () {
                    duong.notify('An error occurred while saving the permission', 'error');
                    duong.stopLoading();
                }
            });
        });
